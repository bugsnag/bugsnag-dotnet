...
namespace Bugsnag.AspNet.Core
{
  ...
  {
    private readonly List<Bugsnag.Middleware> _internalMiddleware; 

    public Client(IOptions<Configuration> configuration, IBreadcrumbs breadcrumbs, ISessionTracker sessionTracker, ITransport transport) : base(configuration.Value, transport, breadcrumbs, sessionTracker)
    {
      DiagnosticExceptionListener.Instance.ConfigureClient(this);
